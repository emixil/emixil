...
const char * const BITCOIN_CONF_FILENAME = "emixil.conf";
const char * const BITCOIN_PID_FILENAME = "emixild.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "emixil" is a composite category enabling all Emixil-related debug output
            if(ptrCategory->count(string("emixil"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\EmixilCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\EmixilCore
    // Mac: ~/Library/Application Support/EmixilCore
    // Unix: ~/.emixilcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "EmixilCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/EmixilCore";
#else
    // Unix
    return pathRet / ".emixilcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty emixil.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override emixil.conf
